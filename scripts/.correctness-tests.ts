...
assert.doesNotMatch(yamlOnlyMetadata.body, /document_id/u);
...
assert.doesNotMatch(legacyOnlyMetadata.body, /Mimora Metadata/u);
...
assert.equal(hasMetadataIssue(mixedSameMetadata, 'metadata_conflict'), false);
...
# Mixed conflict
`);
const documentIdConflict = mixedConflictMetadata.issues.find(
...
assert.ok(documentIdConflict);
assert.equal(documentIdConflict.details?.selectedValue, 'DOC-2026-0004');
...
`);
assert.equal(
  hasMetadataIssue(missingDocumentIdMetadata, 'missing_document_id'),
  true,
);
...
`);
assert.equal(
  hasMetadataIssue(invalidDocumentIdMetadata, 'invalid_document_id'),
  true,
);
...
`);
assert.equal(
  hasMetadataIssue(invalidWorkspaceIdMetadata, 'invalid_workspace_id'),
  true,
);
...
const normalizedRecentChatSessions: ChatSessions = Object.fromEntries(
  Object.entries(recentChatSessions).map(([workspaceId, messages]) => [
    workspaceId,
    [
      {
...
const populatedRecentChatsHtml = renderToStaticMarkup(
  createElement(RecentChatsView, {
    chatSessions: normalizedRecentChatSessions,
    onDeleteSession: () => undefined,
    onOpenSession: () => undefined,
    onRenameSession: () => undefined,
    workspaces: [
